...
To simulate a crash of a server, you can kill the JVM process in the _refarch-hq1_ docker container. Enter the _refarch-hq1_ container with `nsenter` as explained above. Then, on the command prompt, find the PID of the java process, and send it a kill signal:
...
. section 2.2 deployment diagram
. shared file system
.. what's the shared file system technology used in this ref arch ?
.. what's our recommended clustered filesystem technology for a production environment ?  (GFS2 + SAN)
. MySQL XA data source
+
Mention that a MySQL XA data source is needed for _RefArchDS_ datasource pool due to the multiple JCA resources involved in the trnx.
...
. mention what the equivalent of $JBOSS_HOME is for FSW images .... i found it, but others may not.
Or just suggest to:  su - jboss once logged in via nsenter
